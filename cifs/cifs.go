...
var re = regexp.MustCompile(`(?m)(?:(?:(?P<SMBID>\d+)\) \\\\(?P<Server>[A-Za-z1-9-.]+)(?P<Share>.+)\nSMBs:\s+(?P<SMB>\d+) Oplocks breaks:\s+(?P<OpLocks>\d+)\nReads:\s+(?P<Reads>\d+) Bytes:\s+(?P<ReadsBytes>\d+)\nWrites:\s+(?P<Writes>\d+) Bytes:\s+(?P<WritesBytes>\d+)\nFlushes:\s+(?P<Flushes>\d+)\nLocks:\s+(?P<Locks>\d+) HardLinks:\s+(?P<Hardlinks>\d+) Symlinks:\s+(?P<Symlinks>\d+)\nOpens:\s+(?P<Opens>\d+) Closes:\s+(?P<Closes>\d+) Deletes:\s+(?P<Deletes>\d+)\nPosix Opens:\s+(?P<PosixOpens>\d+) Posix Mkdirs:\s+(?P<PosixMkdirs>\d+)\nMkdirs:\s+(?P<Mkdirs>\d+) Rmdirs:\s+(?P<Rmdirs>\d+)\nRenames:\s+(?P<Renames>\d+) T2 Renames\s+(?P<T2Renames>\d+)\nFindFirst:\s+(?P<FindFirst>\d+) FNext\s+(?P<FNext>\d+) FClose\s+(?P<FClose>\d+)|(?P<SMB3ID>\d+)\) \\\\(?P<SMB3Server>[A-Za-z1-9-.]+)(?P<SMB3Share>.+)\nSMBs:\s+(?P<SMB3>\d+)\nNegotiates:\s+(?P<NegotiatesSent>\d+) (?:sent|total)\s+(?P<NegotiatesFailed>\d+) failed\nSessionSetups:\s+(?P<SessionSetupsSent>\d+) (?:sent|total)\s+(?P<SessionSetupsFailed>\d+) failed\nLogoffs:\s+(?P<LogoffsSent>\d+) (?:sent|total)\s+(?P<LogoffsFailed>\d+) failed\nTreeConnects:\s+(?P<TreeConnectsSent>\d+) (?:sent|total)\s+(?P<TreeConnectsFailed>\d+) failed\nTreeDisconnects:\s+(?P<TreeDisconnectsSent>\d+) (?:sent|total)\s+(?P<TreeDisconnectsFailed>\d+) failed\nCreates:\s+(?P<CreatesSent>\d+) (?:sent|total)\s+(?P<CreatesFailed>\d+) failed\nCloses:\s+(?P<ClosesSent>\d+) (?:sent|total)\s+(?P<ClosesFailed>\d+) failed\nFlushes:\s+(?P<FlushesSent>\d+) (?:sent|total)\s+(?P<FlushesFailed>\d+) failed\nReads:\s+(?P<ReadsSent>\d+) (?:sent|total)\s+(?P<ReadsFailed>\d+) failed\nWrites:\s+(?P<WritesSent>\d+) (?:sent|total)\s+(?P<WritesFailed>\d+) failed\nLocks:\s+(?P<LocksSent>\d+) (?:sent|total)\s+(?P<LocksFailed>\d+) failed\nIOCTLs:\s+(?P<IOCTLsSent>\d+) (?:sent|total)\s+(?P<IOCTLsFailed>\d+) failed\nCancels:\s+(?P<CancelsSent>\d+) (?:sent|total)\s+(?P<CancelsFailed>\d+) failed\nEchos:\s+(?P<EchosSent>\d+) (?:sent|total)\s+(?P<EchosFailed>\d+) failed\nQueryDirectories:\s+(?P<QueryDirectoriesSent>\d+) (?:sent|total)\s+(?P<QueryDirectoriesFailed>\d+) failed\nChangeNotifies:\s+(?P<ChangeNotifiesSent>\d+) (?:sent|total)\s+(?P<ChangeNotifiesFailed>\d+) failed\nQueryInfos:\s+(?P<QueryInfosSent>\d+) (?:sent|total)\s+(?P<QueryInfosFailed>\d+) failed\nSetInfos:\s+(?P<SetInfosSent>\d+) (?:sent|total)\s+(?P<SetInfosFailed>\d+) failed\nOplockBreaks:\s+(?P<OpLockBreaksSent>\d+) (?:sent|total)\s+(?P<OpLockBreaksFailed>\d+) failed)+)`)

// Newer format regex - for kernels with different stats format that includes TreeConnects without Negotiates/SessionSetups
var reNewer = regexp.MustCompile(`(?m)(?P<SMB3ID>\d+)\) \\\\(?P<SMB3Server>[A-Za-z1-9-.]+)(?P<SMB3Share>.+)\nSMBs:\s+(?P<SMB3>\d+)\n(?:Bytes read:\s+\d+\s+Bytes written:\s+\d+\n)?(?:Open files:.*\n)?TreeConnects:\s+(?P<TreeConnectsSent>\d+) (?:sent|total)\s+(?P<TreeConnectsFailed>\d+) failed\nTreeDisconnects:\s+(?P<TreeDisconnectsSent>\d+) (?:sent|total)\s+(?P<TreeDisconnectsFailed>\d+) failed\nCreates:\s+(?P<CreatesSent>\d+) (?:sent|total)\s+(?P<CreatesFailed>\d+) failed\nCloses:\s+(?P<ClosesSent>\d+) (?:sent|total)\s+(?P<ClosesFailed>\d+) failed\nFlushes:\s+(?P<FlushesSent>\d+) (?:sent|total)\s+(?P<FlushesFailed>\d+) failed\nReads:\s+(?P<ReadsSent>\d+) (?:sent|total)\s+(?P<ReadsFailed>\d+) failed\nWrites:\s+(?P<WritesSent>\d+) (?:sent|total)\s+(?P<WritesFailed>\d+) failed\nLocks:\s+(?P<LocksSent>\d+) (?:sent|total)\s+(?P<LocksFailed>\d+) failed\nIOCTLs:\s+(?P<IOCTLsSent>\d+) (?:sent|total)\s+(?P<IOCTLsFailed>\d+) failed\n(?:Cancels:\s+(?P<CancelsSent>\d+) (?:sent|total)\s+(?P<CancelsFailed>\d+) failed\n)?(?:Echos:\s+(?P<EchosSent>\d+) (?:sent|total)\s+(?P<EchosFailed>\d+) failed\n)?QueryDirectories:\s+(?P<QueryDirectoriesSent>\d+) (?:sent|total)\s+(?P<QueryDirectoriesFailed>\d+) failed\nChangeNotifies:\s+(?P<ChangeNotifiesSent>\d+) (?:sent|total)\s+(?P<ChangeNotifiesFailed>\d+) failed\nQueryInfos:\s+(?P<QueryInfosSent>\d+) (?:sent|total)\s+(?P<QueryInfosFailed>\d+) failed\nSetInfos:\s+(?P<SetInfosSent>\d+) (?:sent|total)\s+(?P<SetInfosFailed>\d+) failed\nOplockBreaks:\s+(?P<OpLockBreaksSent>\d+) (?:sent|total)\s+(?P<OpLockBreaksFailed>\d+) failed`)
...
func (stats *ClientStats) parseSMBBlocks(file string) {
	...
	// If no matches with original regex, try the newer format
	if len(matches) == 0 {
		matches = reNewer.FindAllStringSubmatch(file, -1)
		// Process newer format matches - they start directly with SMB3 format
		for _, match := range matches {
			block := &Block{
				Server:  match[2], // SMB3Server position
				Share:   match[3], // SMB3Share position
				Metrics: []uint64{},
			}
			// match[4] is where the metrics start for newer format (starting with SMB3 total, then TreeConnectsSent)
			// We need to add 0 for Negotiates and SessionSetups (not present in newer format)
			block.Metrics = append(block.Metrics, 0) // Placeholder for match[4] which is SMB3 count
			// Now parse the actual metrics starting from position 4
