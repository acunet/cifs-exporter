...
func NewCIFSCollector(debug bool) *CIFSCollector {
...
func (c *CIFSCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		if c.debug {
			log.Printf("Error reading CIFS stats: %v", err)
		}
	...
	// Debug logging
	if c.debug {
		log.Printf("CIFS Stats - Targets: %d, Blocks: %d, Sessions: %d",
			stats.Header.Targets, len(stats.Blocks), stats.Header.CIFSSession)
	}

	// Check if there are actually any CIFS shares mounted by checking both
	// the header targets count and the presence of blocks
	if stats.Header.Targets == 0 || len(stats.Blocks) == 0 {
		if c.debug {
			log.Printf("No CIFS shares detected (Targets=%d, Blocks=%d)", stats.Header.Targets, len(stats.Blocks))
		}
		ch <- prometheus.MustNewConstMetric(c.up, prometheus.GaugeValue, float64(0))
		return
	}
	if c.debug {
		log.Printf("CIFS shares detected - setting cifs_up to 1")
	}
